...
async saveTraders(chatId, userTradersObject) {
   ...
   async saveSettings(settings) {
    ...
    async loadWithdrawalHistory() {
        try {
            const data = await fs.readFile(WITHDRAWAL_HISTORY_FILE, 'utf8');
            ...
            if (error.code === 'ENOENT') {
                ...
                console.error('DataManager: Error loading withdrawal history:', error);
                return [];
            }
        }
    }
    ...
    async recordWithdrawal(data) {
        try {
            let history = await this.loadWithdrawalHistory();
            history.push(data);
